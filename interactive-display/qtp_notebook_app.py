...
def _show_recent_runs(base=OUTPUT_DIR, limit=5):
    ...
    for path in runs:
        ...
        png = next(iter(sorted(path.glob("*_pattern.png"))), None)
        summary = next(iter(sorted(path.glob("*_pattern_summary.json"))), None)
        movie = next(iter(sorted(path.glob("*_pattern.mp4"))), None)
...
def _verification_summary_markdown(label, ok, rows):
    groups = {
        ...
        "Required pattern checks": "display_pattern_checks_",
        "Pattern verification": "display_pattern_verification_",
        "Result-scope checks": "display_scope_",
    }
...
def run_display(
    ...
    figure_path = display_dir / f"{prefix}_pattern.png"
    thumbnail_path = display_dir / f"{prefix}_thumb.jpg"
    movie_path = display_dir / f"{prefix}_pattern.mp4"
    summary_path = display_dir / f"{prefix}_pattern_summary.json"
    ...
    display(
        Markdown(
            ...
            "Preparing the pattern figure and numerical summary..."
        )
    )
    summary = disp.make_case_figure(
        result,
        figure_path,
        title=f"{case.capitalize()} reference pattern",
    )
    ...
    display(
        HTML(
            f'<img src="{html.escape(_relative_href(figure_path))}" alt="{html.escape(case.capitalize())} reference pattern and radial spectrum" style="max-width:100%;height:auto;">'
        )
    )
