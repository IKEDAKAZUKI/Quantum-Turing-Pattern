...
def check_required(version: str, errors: list[str]) -> None:
    ...
    expected_display_files = {
        "display_exhibit_manifest.csv",
        "display_movie_manifest.csv",
        "display_verification_report.csv",
    }
    for case in ("spot", "labyrinth", "stripe"):
        expected_display_files.update(
            {
                f"display_{case}_pattern.png",
                f"display_{case}_pattern_summary.json",
                f"display_{case}_pattern.mp4",
                f"display_{case}_thumb.jpg",
            }
        )
        ...
        summary_path = display / f"display_{case}_pattern_summary.json"
        if summary_path.is_file():
            try:
                ...
                if summary.get("movie_file", {}).get("path") != f"display_{case}_pattern.mp4":
                    errors.append(f"unexpected movie path in {summary_path.name}")

    if display.is_dir():
        actual_display_files = {
            path.relative_to(display).as_posix()
            for path in display.rglob("*")
            if path.is_file()
        }
        missing = sorted(expected_display_files - actual_display_files)
        extra = sorted(actual_display_files - expected_display_files)
        if missing:
            errors.append("display files missing from the release set: " + ", ".join(missing))
        if extra:
            errors.append("unexpected files in display/: " + ", ".join(extra))
