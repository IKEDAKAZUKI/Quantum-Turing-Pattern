"""Museum exhibit interface for the bundled reference movies."""
...
def _case_metadata(display_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for key, label in CASES:
        summary_path = display_dir / f"display_{key}_strong_qtp_summary.json"
...
def exhibit_html(
    ...
    *,
    ...
    The player uses the bundled reference movies and reports physical simulation
    time. Set ``embed_media=False`` to use relative movie paths.
    """
    ...
    sources = {
        key: _video_source(
            display_dir / f"display_{key}_exhibit.mp4",
...
<style>
#{uid}.qtp-exhibit {{
  width:min(98vw,1580px); height:calc(100vh - 8px); max-height:1080px;
  margin:0 auto; padding:4px; box-sizing:border-box; overflow:hidden;
  display:flex; flex-direction:column; gap:8px;
  font-family:"Times New Roman",Times,"TeX Gyre Termes","Nimbus Roman",serif; color:#171717;
}}
#{uid} .qtp-title {{ flex:0 0 auto; text-align:center; margin:0; }}
#{uid} .qtp-title h1 {{ margin:0; font-size:clamp(28px,2.6vw,48px); line-height:1.02; }}
#{uid} .qtp-title p {{ margin:3px 0 0; color:#4a4a4a; font-size:clamp(14px,1.05vw,18px); }}
#{uid} .qtp-toolbar {{
  flex:0 0 auto; display:grid;
  grid-template-columns:auto auto auto auto auto minmax(260px,1fr);
  align-items:center; justify-content:center; gap:8px;
  padding:8px 10px; border:1px solid #d6d6d6; border-radius:14px;
  background:#fafafa; box-shadow:0 2px 9px rgba(0,0,0,.06);
}}
#{uid} button {{
  min-height:50px; min-width:118px; border:2px solid #777; border-radius:11px;
  background:white; padding:7px 15px; font-size:clamp(16px,1.05vw,20px);
...
#{uid} input[type="range"] {{
  ...
  min-height:48px; margin:0; background:transparent; cursor:pointer; touch-action:pan-x;
}}
...
#{uid} .qtp-video-wrap {{
  ...
  overflow:hidden; border-radius:14px; border:1px solid #d6d6d6; background:#fff;
}}
...
<div class="qtp-title">
  <h1>Quantum Turing Patterns</h1>
  <p>Choose a pattern and replay its formation through physical simulation time.</p>
</div>
