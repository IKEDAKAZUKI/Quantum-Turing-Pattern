#!/usr/bin/env python3
"""Reproduce the three reference cases and their display assets."""
...
def reproduce_assets(output_dir: Path) -> None:
    ...
    diagnostic_vmax = max(float(np.nanmax(result["E_frames"])) for _, _, result in results)
    pattern_vmax = max(abs(float(disp.VMIN)), abs(float(disp.VMAX)))

    for case, prefix, result in results:
        print(f"Writing figures and movies for {case.capitalize()}...")
        figure_path = output_dir / f"{prefix}_strong_qtp.png"
        movie_path = output_dir / f"{prefix}_strong_qtp.mp4"
        summary_path = output_dir / f"{prefix}_strong_qtp_summary.json"
        ...
        disp.make_case_movie(
            ...
            diagnostic_scale_scope="bundled_reference_movies",
        )
        ...
        disp.make_exhibit_movie(
            result,
            output_dir / f"{prefix}_exhibit.mp4",
